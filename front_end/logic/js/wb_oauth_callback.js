...
var vm = new Vue({
    ...
    mounted: function(){
        ...
        axios.get(this.host + '/oauth/wb/user/?code=' + code, {
                responseType: 'json',
            ...
            .then(response => {
                if (response.data.user_id){
                    ...
                    localStorage.oauth_access_token = response.data.oauth_access_token;
                    var state = this.get_query_string('state');
                    location.href = state;
                } else {
                    // 用户未绑定
                    this.access_token = response.data.access_token;
                    this.oauth_access_token = response.data.oauth_access_token;
    ...
    methods: {
        ...
        send_sms_code: function(){
            ...
            axios.get(this.host + '/sms_codes/' + this.mobile + '/?text=' + this.image_code+'&image_code_id='+ this.image_code_id, {
                ...
                .then(response => {
                    // 表示后端发送短信成功
                    // 倒403计时60秒，60秒后允许用户再次点击发送短信验证码的按钮
                    var num = 60;
                    // 设置一个计时器
                    var t = setInterval(() => {
                        if (num == 1) {
        ...
        on_submit: function(){
            ...
            this.check_sms_code();
             ...
             if(this.error_password == false && this.error_phone == false && this.error_sms_code == false) {
                axios.post(this.host + '/oauth/wb/user/', {
